...
class ValidationCallback(object):
  ...
  def __call__(self, model, step):
    ...
    y_pred = model.predict(self.dataset).copy()

    zero_ids = np.vstack(np.where(self.dataset.w==0)).T
    for i,j in zero_ids:
      y_pred[i,j] = self.dataset.y[i,j]

    score = self.metrics[0].compute_metric(self.dataset.y, y_pred)
    ...
    print("MESSAGE: train score: {} / validation score: {} / at epoch: {} / {} / elapsed time {}s /".format(
        "%.4g" % score, "%.4g" % score, self.epoch+1, "device", int(time.time()-self.start_moment)))
    self.epoch += 1
